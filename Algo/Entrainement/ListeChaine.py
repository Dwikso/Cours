...
class ListeChainee :
    ...
    def compte_negatif(self):
        
        def count_negatives(maillon, cpt=0):
            if maillon == None:
                return cpt
            elif maillon.get() < 0:
                return count_negatives(maillon.suivant(), cpt + 1)
            else:
                return count_negatives(maillon.suivant(), cpt)
        
        return count_negatives(self.__tete)
    
    def supprimer_inferieur_a(self, val):
        if self.__tete is None:
            return
        elif self.__tete.get() < val:
            self.__tete = self.__tete.suivant()
            self.supprimer_inferieur_a(val)
        else:
            courant = self.__tete
            while courant.suivant() != None:
                if courant.suivant().get() < val:
                    courant.set_suiv(courant.suivant().suivant())
                else:
...
l.append(1)
l.append(8)
print(l.compte_negatif())
print(l.supprimer_inferieur_a(2))
print(l)
